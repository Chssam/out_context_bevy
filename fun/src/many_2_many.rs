...
#[component(immutable)]
pub struct ModNotif(#[entities] EntityHashSet);

impl OutputEntity for ModNotif {
...
#[derive(Component, Default, Clone, Deref, MapEntities)]
#[component(immutable)]
pub struct GetModNotif(#[entities] EntityHashSet);

impl OutputEntity for GetModNotif {
	fn new_self(entity_set: EntityHashSet) -> Self {
		Self(entity_set)
	}
...
pub fn m_2_m<T: OutputEntity, U: OutputEntity>(world: &mut World) {
	world.register_component_hooks::<T>().on_remove(
		|mut world: DeferredWorld, HookContext { entity, .. }: HookContext| {
			let ent_mut = world.entity(entity);
			let mut mod_notif = ent_mut.get::<T>().cloned().unwrap().entity_set();
			mod_notif.drain().for_each(|entity_notif| {
				world
					.commands()
					.queue(RemoveMod::<T, U>::new(entity, entity_notif));
			});
		},
	);

	world.register_component_hooks::<U>().on_remove(
		|mut world: DeferredWorld, HookContext { entity, .. }: HookContext| {
			let ent_mut = world.entity(entity);
			let mut mod_notif = ent_mut.get::<U>().cloned().unwrap().entity_set();
			mod_notif.drain().for_each(|entity_mod| {
				world
					.commands()
					.queue(RemoveMod::<T, U>::new(entity_mod, entity));
			});
		},
	);
}
	...
	fn apply(self, world: &mut World) {
		{
			...
			world
				.commands()
				.entity(self.moder)
				.insert(T::new_self(mod_notif));
		}

		{
			let mut get_notif = world
				.get::<U>(self.entity)
				.cloned()
				.unwrap_or_default()
				.entity_set();
...
mod tests {
	...
	fn verify_total() {
		let mut app = App::new();
		app.add_systems(PreStartup, m_2_m::<ModNotif, GetModNotif>)
			.add_systems(Startup, setup::<ModNotif, GetModNotif>)
	...
	fn setup<T: OutputEntity, U: OutputEntity>(world: &mut World) {
		...
		world
			.commands()
			.queue(ShareMod::<T, U>::new(modder_0, ent_3)); // Valid
		world
			.commands()
			.queue(ShareMod::<T, U>::new(modder_1, ent_3)); // Valid
		world
			.commands()
			.queue(ShareMod::<T, U>::new(modder_0, ent_4)); // Valid
		world
			.commands()
			.queue(ShareMod::<T, U>::new(modder_0, ent_4)); // Duplicate
		world
			.commands()
			.queue(ShareMod::<T, U>::new(modder_0, ent_5)); // Valid
		world
			.commands()
			.queue(ShareMod::<T, U>::new(modder_1, ent_6)); // Valid

		// Remove 2 but only 1 is matched, so total valid now 4
		world
			.commands()
			.queue(RemoveMod::<T, U>::new(modder_0, ent_4)); // Valid
		world
			.commands()
			.queue(RemoveMod::<T, U>::new(modder_0, ent_6)); // Unmatched

		// Below here all not count toward
		world
			.commands()
			.queue(ShareMod::<T, U>::new(modder_2, ent_3)); // Mod will be despawned so not in GetModNotif
		world
			.commands()
			.queue(ShareMod::<T, U>::new(modder_2, ent_6)); // Mod will be despawned so not in GetModNotif

		world
			.commands()
			.queue(ShareMod::<T, U>::new(modder_0, ent_7)); // Entity 7 will be despawned so not in ModNotif
